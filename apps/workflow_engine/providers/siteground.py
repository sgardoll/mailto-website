...
class SiteGroundProvider:
    ...
    def deploy(self, build_result: BuildResult, config: dict) -> DeployResult:
        ...
        engine_result = _deploy_workflow_engine_if_enabled(config)
        if engine_result and not engine_result["ok"]:
            log.warning("Workflow engine deploy failed: %s", engine_result.get("error"))
